...
def generate():
    ...
    try:
        ...
        # Step 3: Detect duplication by checking if content is roughly doubled
        # If text is duplicated, second half will be very similar to first half
        lines = [line for line in cleaned.split('\n') if line.strip()]
        
        if len(lines) > 20:  # Only check if substantial content
            # Check if second half mirrors first half (typical of duplication)
            midpoint = len(lines) // 2
            
            # Count how many lines from first half appear in second half
            first_half_lines = set(lines[:midpoint])
            second_half_lines = set(lines[midpoint:])
            
            # Calculate overlap
            overlap = len(first_half_lines & second_half_lines)
            overlap_ratio = overlap / len(first_half_lines) if first_half_lines else 0
            
            # If >70% overlap, likely duplicated - take first half only
