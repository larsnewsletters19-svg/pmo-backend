...
def generate():
    ...
    try:
        ...
        # Claude now returns only one markdown version
        # Clean any accidental version headers
        ...
        # Use cleaned content for both versions
        # Word version keeps markdown for .docx generation
        word_text = cleaned_content
        
        # OneNote version is simplified (remove markdown)
        onenote_version = re.sub(r'#{1,3}\s+', '', cleaned_content)  # Remove header markers
        onenote_version = re.sub(r'\*\*(.+?)\*\*', r'\1', onenote_version)  # Remove bold
        onenote_version = re.sub(r'\|[^\n]+\|', '', onenote_version)  # Remove tables
        onenote_version = re.sub(r'\n\s*[-:]+\s*\n', '\n', onenote_version)  # Remove table separators
        onenote_version = re.sub(r'\n{3,}', '\n\n', onenote_version)  # Clean excess newlines
