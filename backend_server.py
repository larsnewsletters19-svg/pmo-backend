...
def generate():
    ...
    try:
        ...
        # Claude may return content in different formats:
        # Format 1: "OneNote version:\n...\n\nWord version:\n..."
        # Format 2: Just markdown content with no labels
        
        # Try to split by "Word version" marker
        word_marker_pattern = r'\n+\s*(?:Word[- ]?version|Word Version|WORD VERSION)\s*:?\s*\n+'
        parts = re.split(word_marker_pattern, content, flags=re.IGNORECASE)
        
        if len(parts) >= 2:
            # Found split - parts[0] is OneNote, parts[1] is Word
            onenote_raw = parts[0]
            word_raw = parts[1] if len(parts) > 1 else parts[0]
            
            # Clean OneNote version header
            onenote_version = re.sub(r'^[\s\n]*(?:OneNote[- ]?version|OneNote Version|ONENOTE VERSION)\s*:?\s*\n+', '', onenote_raw, flags=re.IGNORECASE).strip()
            
            # Word version should already be clean, but ensure no header remains
            word_text = re.sub(r'^[\s\n]*(?:Word[- ]?version|Word Version|WORD VERSION)\s*:?\s*\n+', '', word_raw, flags=re.IGNORECASE).strip()
            
        else:
            # No clear split found - content might just be one version
            # Check if content has markdown (then it's probably word version)
            if any(marker in content for marker in ['# ', '## ', '### ', '**', '- ', '* ']):
                # Has markdown - use as word version
                word_text = content.strip()
                # Create plain text version for onenote
                onenote_version = re.sub(r'#+ ', '', content)  # Remove markdown headers
                onenote_version = re.sub(r'\*\*(.+?)\*\*', r'\1', onenote_version)  # Remove bold
            else:
                # Plain text - use for both
                onenote_version = content.strip()
                word_text = content.strip()
