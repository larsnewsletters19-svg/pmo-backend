...
def generate():
    ...
    try:
        ...
        # AGGRESSIVE CLEANING: Claude ignores prompt and creates duplicates anyway
        # Force single version by removing all version headers and splits
        
        # Step 1: Remove all version headers (case insensitive, any format)
        ...
        # Step 3: If content still has duplicates, take only first half (before any ---or second version marker)
        # Split by strong indicators of duplication
        parts = re.split(r'\n+(?:#{1,2}\s*(?:Word|OneNote)[- ]?version|={3,}|-{3,})\s*\n+', cleaned, flags=re.IGNORECASE)
        if len(parts) > 1:
            # Take first part only (before duplication)
        ...
        # Create simplified OneNote version (remove markdown)
