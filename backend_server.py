...
def generate():
    ...
    try:
        ...
        # SIMPLIFIED CLEANING: Remove version headers and find duplicate H1
        
        # Step 1: Remove ONLY version headers (OneNote-version, Word-version)
        ...
        # Step 3: Find first H1 (document title)
        first_h1_match = re.search(r'^(#\s+.+?)$', cleaned, re.MULTILINE)
        
        if first_h1_match:
            first_h1 = first_h1_match.group(0)
            
            # Find ALL occurrences of this EXACT H1
            h1_positions = [m.start() for m in re.finditer(re.escape(first_h1), cleaned)]
            
            # If H1 appears more than once, it's duplicated content
            if len(h1_positions) > 1:
                # Take everything BEFORE the second occurrence
                cleaned = cleaned[:h1_positions[1]].strip()
