...
def generate():
    ...
    try:
        ...
        document_title = title_match.group(0) if title_match else None
        ...
        if len(paragraphs) > 5:  # Only check substantial content
            ...
            # Look for where document title appears AGAIN (common duplication pattern)
            if document_title:
                # Find all occurrences of title
                title_occurrences = [i for i, p in enumerate(paragraphs) if document_title in p]
                
                if len(title_occurrences) > 1:
                    # Second occurrence of title = start of duplication
                    # Take everything BEFORE second occurrence
                    cleaned = '\n\n'.join(paragraphs[:title_occurrences[1]])
                else:
                    # No repeated title, use word overlap method
