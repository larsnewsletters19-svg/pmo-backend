...
CORS(app, resources={
    r"/api/*": {
        "origins": [
            "https://larsnewsletters19-svg.github.io",
            "http://localhost:*",
            "http://127.0.0.1:*"
        ],
...
def generate():
    ...
    try:
        ...
        sections = re.split(r'(?:Word[- ]version|Word Version|WORD VERSION)', content, flags=re.IGNORECASE)
        onenote_version = re.sub(r'(?:OneNote[- ]version|OneNote Version|ONENOTE VERSION)', '', sections[0], flags=re.IGNORECASE).strip()
        word_text = sections[1].strip() if len(sections) > 1 else content
