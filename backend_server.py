...
def generate():
    ...
    try:
        ...
        # PRESERVE H1 title before processing
        title_match = re.search(r'^(#\s+.+?)$', cleaned, re.MULTILINE)
        preserved_title = title_match.group(0) + '\n\n' if title_match else ''
        
        # Remove H1 from content for duplicate detection (we'll add it back)
        content_without_title = re.sub(r'^#\s+.+?\n+', '', cleaned, count=1, flags=re.MULTILINE)
        
        # Split content (without title) into paragraphs
        paragraphs = [p.strip() for p in content_without_title.split('\n\n') if p.strip()]
        
        if len(paragraphs) > 5:  # Only check substantial content
            # Find likely duplication point
            # Look for where title text appears AGAIN in content (indicates duplication)
            if title_match:
                title_text = strip_formatting(title_match.group(0))
                
                # Find paragraphs that contain similar title text (duplication indicator)
                title_occurrences = [i for i, p in enumerate(paragraphs) 
                                    if title_text in strip_formatting(p) and len(strip_formatting(p)) < len(title_text) * 3]
                
                if len(title_occurrences) > 0:
                    # Title appears again = duplication starts there
                    # Take everything BEFORE first re-occurrence
                    paragraphs = paragraphs[:title_occurrences[0]]
                else:
                    # No title repetition, use word overlap method
                ...
                # No title found, use word overlap
                midpoint = len(paragraphs) // 2
                first_half_stripped = ' '.join([strip_formatting(p) for p in paragraphs[:midpoint]])
                second_half_stripped = ' '.join([strip_formatting(p) for p in paragraphs[midpoint:]])
        ...
        # Reconstruct content WITH preserved title
