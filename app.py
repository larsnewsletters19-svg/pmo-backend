# AI PMO Generator Backend - v8.7.0
...
def serve_js(filename):
    """Servera JavaScript filer från js/ mappen (v8.7.0)"""
...
def generate():
    ...
    try:
        ...
        # Step 2.5: Remove leaked formatting instructions (v8.7.4+)
        # Remove "Word-version:" headers and following text
        cleaned = re.sub(r'Word-version.*?(?:\n\n|\Z)', '', cleaned, flags=re.IGNORECASE | re.DOTALL)
        # Remove **[Word-formatted...] pattern
        cleaned = re.sub(r'\*\*\[.*?Word.*?formaterad.*?\]\*\*', '', cleaned, flags=re.IGNORECASE | re.DOTALL)
        cleaned = re.sub(r'\[.*?Word.*?formaterad.*?\]', '', cleaned, flags=re.IGNORECASE | re.DOTALL)
        # Remove any remaining formatting instruction patterns
        cleaned = re.sub(r'\[Ett Word-dokument.*?\]', '', cleaned, flags=re.DOTALL)
        cleaned = re.sub(r'Word Format Instructions:.*?(?:\n\n|\Z)', '', cleaned, flags=re.DOTALL)
        # Remove internal instruction markers
        cleaned = re.sub(r'=== INTERNAL.*?=== END INTERNAL.*?===', '', cleaned, flags=re.IGNORECASE | re.DOTALL)
